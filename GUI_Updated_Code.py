...
# SMART BAND DETECTION FROM METADATA
...
def preprocess_for_model(composite_image):
    """
    Apply per-band standardization preprocessing.
    Returns: (preprocessed_image, preprocessing_log)
    """
    log_lines = []
    log_lines.append("\n[PREPROCESSING] Applying per-band standardization:")
    preprocessed = np.zeros_like(composite_image, dtype=np.float32)
    
    for band_idx in range(8):
        band = composite_image[:, :, band_idx].astype(np.float32)
        
        # Handle NaN and Inf values
        valid_mask = np.isfinite(band)
        if not np.any(valid_mask):
            msg = f"   Band {band_idx+1}: ALL NaN/Inf - filling with zeros"
            log_lines.append(msg)
            preprocessed[:, :, band_idx] = 0.0
            continue
        
        # Calculate band statistics
        band_mean = np.nanmean(band[valid_mask])
        band_std = np.nanstd(band[valid_mask])
        ...
        if band_std > 1e-8:
            standardized_band = (band - band_mean) / band_std
        else:
            standardized_band = band - band_mean
        
        preprocessed[:, :, band_idx] = standardized_band
        
        # Verify
        std_mean = np.mean(preprocessed[:, :, band_idx])
        std_std = np.std(preprocessed[:, :, band_idx])
        std_min = np.min(preprocessed[:, :, band_idx])
        std_max = np.max(preprocessed[:, :, band_idx])
        
        msg = f"   Band {band_idx+1}: mean={band_mean:+.6f}, std={band_std:.6f} -> mean={std_mean:+.6f}, std={std_std:.6f}, range=[{std_min:+.4f}, {std_max:+.4f}]"
        log_lines.append(msg)
...
def load_composite_image_8_bands(image_path):
    ...
    try:
        ...
        composite_paths = [
            image_path.replace('.tif', '_composite.tif').replace('.tiff', '_composite.tiff'),
            image_path.replace('.tif', '_processed_composite.tiff').replace('.tiff', '_processed_composite.tiff')
        ]
        
        # Also try in same directory with basename
        base_name = os.path.splitext(os.path.basename(image_path))[0]
        dir_path = os.path.dirname(image_path)
        composite_paths.append(os.path.join(dir_path, f"{base_name}_processed_composite.tiff"))
        composite_paths.append(os.path.join(dir_path, f"{base_name}_composite.tiff"))
...
def display_model_input_preview(img_path):
    ...
    global model_preview_canvas
    
    if model_preview_canvas is None:
        return
    ...
    try:
        ...
        try:
            with rasterio.open(img_path) as src:
                ...
                # Create PIL image
                pil_img = PILImage.fromarray(data_8bit, mode='L')
        except:
            # If rasterio fails, try PIL directly
            pil_img = PILImage.open(img_path)
        
        # Resize to fit canvas
        canvas_width = model_preview_canvas.winfo_width() or 400
        canvas_height = model_preview_canvas.winfo_height() or 300
        ...
        x = canvas_width // 2
        y = canvas_height // 2
        model_preview_canvas.create_image(x, y, image=photo, anchor='center')
        model_preview_canvas.image = photo  # Keep reference
        
    except Exception as e:
        messagebox.showerror("Preview Error", f"Failed to display image:\n{str(e)}")
    ...
    try:
        ...
        for i, filename in enumerate(image_files, 1):
            ...
            try:
                ...
                mask_path = os.path.join(model_output_folder, f"{base_name}_segmentation_mask.png")
                mask_uint8 = (binary_mask * 255).astype(np.uint8)
                PILImage.fromarray(mask_uint8).save(mask_path)
                
                result_dict['mask_path'] = mask_path
...
def select_output_folder():
    ...
    if output_folder_path:
        ...
        set_status(f"✓ Output folder selected: {os.path.basename(output_folder_path)}", "success")
    else:
        output_folder_display.config(
            text="Not selected",
...
def upload_model():
    ...
    try:
        ...
        model_details_text.delete('1.0', 'end')
        model_details_text.insert('1.0', ''.join(model_info))
...
left_column_container = tk.Frame(content_frame, bg=COLORS['background'], bd=0, highlightthickness=0)
left_column_container.grid(row=0, column=0, sticky='nsew', padx=(0, 10))

left_canvas = tk.Canvas(left_column_container, bg=COLORS['background'], highlightthickness=0)
left_scrollbar = tk.Scrollbar(left_column_container, orient="vertical", command=left_canvas.yview)

left_column = tk.Frame(left_canvas, bg=COLORS['background'])
...
if IS_WINDOWS or IS_MAC:
    ...
    def on_left_mousewheel_linux(event):
        if event.num == 4:
            left_canvas.yview_scroll(-1, "units")
        elif event.num == 5:
            left_canvas.yview_scroll(1, "units")
    left_canvas.bind_all("<Button-4>", on_left_mousewheel_linux)
    left_canvas.bind_all("<Button-5>", on_left_mousewheel_linux)
...
# ─── 3. CALCULATION CARD ───
calc_card_outer, calc_card = _make_card(left_column, "Calculation", "⚙️")
calc_card_outer.pack(fill='x', pady=(0, 12))
...
# ─── 4. ACTIONS CARD ───
action_card_outer, action_card = _make_card(left_column, "Actions", "🚀")
action_card_outer.pack(fill='x', pady=(0, 12))
...
def _build_model_tab():
    ...
    # Main scrollable frame for model tab
    model_scroll_canvas = tk.Canvas(model_tab, bg=COLORS['card_bg'], highlightthickness=0)
    model_scrollbar = tk.Scrollbar(model_tab, orient="vertical", command=model_scroll_canvas.yview)
    model_scroll_frame = tk.Frame(model_scroll_canvas, bg=COLORS['card_bg'])
    
    model_scroll_frame.bind(
        "<Configure>",
        lambda e: model_scroll_canvas.configure(scrollregion=model_scroll_canvas.bbox("all"))
    )
    
    canvas_window = model_scroll_canvas.create_window((0, 0), window=model_scroll_frame, anchor="nw")
    
    # Make frame expand to canvas width
    def _on_canvas_configure(event):
        model_scroll_canvas.itemconfig(canvas_window, width=event.width)
    model_scroll_canvas.bind('<Configure>', _on_canvas_configure)
    
    model_scroll_canvas.configure(yscrollcommand=model_scrollbar.set)
    
    model_scroll_canvas.pack(side="left", fill="both", expand=True, padx=6, pady=6)
    model_scrollbar.pack(side="right", fill="y")

    # ===== SECTION 1: Model Upload =====
    model_card_outer, model_card = _make_card(model_scroll_frame, "Machine Learning Model", "🤖")
    ...
    tk.Button(
        model_card,
        text="📤  Upload Keras Model  (.keras / .h5)",
        font=_FONT_BTN,
        bg=COLORS['primary'],
        fg='#ffffff',
        activebackground=COLORS['primary_hover'],
    ...
    model_details_text = tk.Text(
        model_card,
        height=8,
        font=("Consolas", 9) if IS_WINDOWS else ("SF Mono", 9) if IS_MAC else ("DejaVu Sans Mono", 9),
        bg=COLORS['details_bg'],
        fg=COLORS['text'],
        insertbackground=COLORS['text'],
        wrap='word',
    ...
    model_details_text.pack(fill='x')
    model_details_text.insert('1.0', "No model loaded yet.\n\nUpload a .keras or .h5 file to see model architecture and details.")
    model_details_text.config(state='disabled')

    # ===== NEW: Model Validation Button =====
    validation_btn_frame = tk.Frame(model_card, bg=COLORS['card_bg'])
    validation_btn_frame.pack(fill='x', pady=(10, 0))
    
    tk.Button(
        validation_btn_frame,
        text="🔍  Validate Model (Quick Test)",
        font=_FONT_BTN,
        bg=COLORS['secondary_bg'],
        fg='#ffffff',
        activebackground=COLORS['secondary_hover'],
        ...
        relief='flat',
        bd=0,
        command=open_validation_window
    ).pack(fill='x')
    
    tk.Label(
        validation_btn_frame,
        text="Test model with a composite to verify it's working correctly",
        font=_FONT_SMALL,
        bg=COLORS['card_bg'],
        fg=COLORS['text_muted'],
        anchor='w',
    ...
    # ===== SECTION 1.5: Sliding Window Configuration =====
    window_config_card_outer, window_config_card = _make_card(model_scroll_frame, "Sliding Window Settings", "⚙️")
    window_config_card_outer.pack(fill='x', pady=(0, 12))

    tk.Label(
        window_config_card,
        text="Configure window size and stride for large images (auto-enabled when image > model input size)",
        font=_FONT_BODY,
        bg=COLORS['card_bg'],
        fg=COLORS['text_muted'],
        justify='left',
    ...
    predict_card_outer, predict_card = _make_card(model_scroll_frame, "Single Image Prediction", "🎯")
    predict_card_outer.pack(fill='x', pady=(0, 12))

    tk.Label(
        predict_card,
        text="Select an image and run prediction",
        font=_FONT_BODY,
        bg=COLORS['card_bg'],
        fg=COLORS['text_muted'],
        anchor='w'
    ...
    tk.Button(
        predict_card,
        text="📷  Select Image for Prediction",
        font=_FONT_BTN,
        bg=COLORS['secondary_bg'],
        fg='#ffffff',
        activebackground=COLORS['secondary_hover'],
        ...
        relief='flat',
        bd=0,
        command=select_model_input_image
    ).pack(fill='x', pady=(0, 10))
    
    # ✨ NEW: Quick check composite button
    def check_current_composite():
        """Check if composite exists for currently selected image"""
        if model_input_image_path is None:
        ...
        if composite_exists:
            messagebox.showinfo(
                "✓ Composite Found",
                f"Image: {os.path.basename(model_input_image_path)}\n\n"
                "✓ 8-band composite detected!\n\n"
                "You can proceed with prediction."
            )
            update_prediction_button_state(True)
        else:
            base_name = os.path.splitext(os.path.basename(model_input_image_path))[0]
            messagebox.showwarning(
                "⚠️ Composite Missing",
                f"Image: {os.path.basename(model_input_image_path)}\n\n"
                "❌ No 8-band composite found!\n\n"
                f"Expected file:\n{base_name}_processed_composite.tiff\n\n"
                "Please run 'All Indices' in the NDSI Calculation tab first."
            )
            update_prediction_button_state(False)
    
    tk.Button(
        predict_card,
        text="🔍  Check Composite Status",
        font=_FONT_SMALL,
        bg=COLORS['card_bg'],
        fg=COLORS['text'],
        ...
        relief='flat',
        bd=1,
        command=check_current_composite
    ).pack(fill='x', pady=(0, 10))

    # Preview canvas
    model_preview_canvas = tk.Canvas(
        predict_card,
        bg=COLORS['canvas_bg'],
        height=200,
        highlightthickness=1,
    ...
    model_preview_canvas.pack(fill='x', pady=(0, 10))

    # ✨ NEW: Composite status indicator
    global _composite_status_label
    _composite_status_label = tk.Label(
        predict_card,
        text="No image selected",
        font=_FONT_SMALL,
        bg=COLORS['card_bg'],
    ...
    _composite_status_label.pack(fill='x', pady=(0, 10))

    # Store prediction button reference globally
    ...
    _prediction_button = tk.Button(
        ...
        highlightbackground='#7aa3fc',
        highlightcolor='#7aa3fc',
        command=run_model_prediction_CORRECTED
    )
    _prediction_button.pack(fill='x', pady=(0, 10))
    ...
    model_result_label.pack(fill='x', pady=(0, 10))
    
    # Save prediction button
    tk.Button(
        predict_card,
        text="💾  Save Prediction Result",
        font=_FONT_BTN,
        bg=COLORS['secondary_bg'],
        fg='#ffffff',
        activebackground=COLORS['secondary_hover'],
        ...
        relief='flat',
        bd=0,
        command=save_prediction_result
    ).pack(fill='x')

    # ===== SECTION 3: Batch Prediction =====
    batch_card_outer, batch_card = _make_card(model_scroll_frame, "Batch Prediction", "📦")
    batch_card_outer.pack(fill='x', pady=(0, 12))

    tk.Label(
        batch_card,
        text="Run predictions on multiple images",
        font=_FONT_BODY,
        bg=COLORS['card_bg'],
        fg=COLORS['text_muted'],
        anchor='w'
    ...
    tk.Button(
        ...
        relief='flat',
        bd=0,
        command=select_model_output_folder
    ).pack(fill='x', pady=(0, 10))
    ...
    tk.Button(
        ...
        highlightbackground='#7aa3fc',
        highlightcolor='#7aa3fc',
        command=run_batch_prediction_CORRECTED
    ).pack(fill='x')
